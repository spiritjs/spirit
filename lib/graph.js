...
graph.addPort = function (type, id, port, metadata) {
  ...
  ports[port] = node;
  this.emit(type + 'Port:added', port, node);
};
...
graph.addInPort = function (id, port, metadata) {
  this.addPort('in', id, port, metadata);
};

/**
 *  Rmeove inPort by port.
 *
 *  @param {String} port 
 *  @api public
 */

graph.removeInPort = function (port) {
  var node = this.inPorts[port];
  if (!node) return;
  delete this.inPorts[port];
  this.emit('inPort:removed', port, node);
};
...
graph.addOutPort = function (id, port, metadata) {
  this.addPort('out', id, port, metadata);
};

/**
 *  Rmeove outPort by port.
 *
 *  @param {String} port 
 *  @api public
 */

graph.removeOutPort = function (port) {
  var node = this.outPorts[port];
  if (!node) return;
  delete this.outPorts[port];
  this.emit('outPort:removed', port, node);
};

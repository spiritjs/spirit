...
var emptyObject = require('./utils').emptyObject;
...
graph.addEdge = function (source, target, metadata) {
  ...
  edge.source = source;
  edge.target = target;
  edge.metadata = Object.create(metadata);
  //edge.id
...
graph.addInitial = function (data, target, metadata) {
  if (!this.getNode(target.id)) return;
  var edge = Object.create(null);
  edge.source = data;
  edge.target = target;
  edge.metadata = metadata || Object.create(null);
...
graph.clear = function () {
  ...
  emptyObject(this.inPorts);
  emptyObject(this.outPorts);
